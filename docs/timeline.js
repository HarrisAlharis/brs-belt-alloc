...
(function(){
  ...
  function packLanes(flightsForBelt){
    ...
    for (const f of items){
      ...
      for (let li=0; li<lanes.length; li++){
        ...
        if (start >= lastEnd){ placedLane = li; break; }
      }
      if (placedLane === -1){
        lanes.push(+new Date(f.end));
        f._lane = lanes.length - 1;
      } else {
        lanes[placedLane] = +new Date(f.end);
        f._lane = placedLane;
      }
    }
  ...
  // ✅ UPDATED buildPuck FUNCTION (top-aligned tweak only)
  function buildPuck(f){
    ...
    // ⬆️ Vertical lane offset (aligned to top line instead of bottom)
    p.style.top = `${f._lane * (laneH + laneGap)}px`;
    p.style.transform = "translateY(-10%)"; // minor lift for top alignment
  ...
  function draw(){
    ...
    for (const b of belts){
      ...
      const contentH = lanesCount * (laneH + laneGap) - laneGap;
