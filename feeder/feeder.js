...
function assignBelts(rowsIn) {
  ...
  // IMPORTANT: sort by start so we replay the day correctly
  ...
  for (const flight of rows) {
    ...
    // domestic always stays 7 if it was set
    if (currentBelt === 7) {
      continue;
    }

    // if FR24 (or later script) already set a good belt, keep it and track it
    ...
    // if none fits, force to the belt that will clear earliest
    if (!placed) {
